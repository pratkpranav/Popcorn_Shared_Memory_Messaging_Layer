	...
	static int ivshmem_send(void __iomem *data_base_addr_parted ,struct pcn_kmsg_message* buf, int handles_id)
	{
		...
		do
		{
			...
			void* readVM = kmalloc(sizeof(int), GFP_KERNEL);
			memcpy_fromio(readVM,data_base_addr_parted - sizeof(int), sizeof(int));
			*p = *((int *)readVM);
			kfree(readVM);
		...
		printk(KERN_INFO "Started Writing Message to Shared Memory from ARM | handles: %d\n", handles_id);

		// spin_lock(&lock[handles_id]);
		memcpy_toio(data_base_addr_parted - sizeof(int), VM1, sizeof(int));
		memcpy_toio(data_base_addr_parted, buf, sizeof(struct pcn_kmsg_message));
		// spin_unlock(&lock[handles_id]);

		printk(KERN_INFO "Done Writing Message to Shared Memory from ARM | handles: %d\n", handles_id);
	...
	static int ivshmem_recv(void __iomem *data_base_addr_parted ,void* buf, int handles_id)
	{
		...
		do
		{
			...
			//printk(KERN_INFO "12\n");
			void* readVM = kmalloc(sizeof(int), GFP_KERNEL);
			//printk(KERN_INFO "Started reading shared memory for receiving | handles_id :%d\n", handles_id);
			memcpy_fromio(readVM,data_base_addr_parted - sizeof(int), sizeof(int));
			//printk(KERN_INFO "Ended reading shared memory for receiving | handles_id :%d\n", handles_id);
			*p = *((int *)readVM);
			//printk(KERN_INFO "15\n");
			//printk(KERN_INFO  "Reading recv %d",  *((int *)readVM));
			kfree(readVM);
			// spin_unlock(&lock[handles_id]);
		}
		while(*p != 1);
		...
		printk(KERN_INFO "Started Reading Message from Shared Memory from ARM | handles: %d\n", handles_id);

		// spin_lock(&lock[handles_id]);
		memcpy_fromio(buf, data_base_addr_parted, sizeof(struct pcn_kmsg_message));
		//printk(KERN_INFO "3\n");
		memcpy_toio(data_base_addr_parted - sizeof(int), OPEN, sizeof(int));
		printk(KERN_INFO "Done Reading Message from Shared Memory from ARM | handles: %d\n", handles_id);
		//printk(KERN_INFO "4\n");
		// spin_unlock(&lock[handles_id]);
	...
	static int recv_handler(void* arg0)
	{
		...
		while(!kthread_should_stop()){
			...
			int ret;
			size_t offset;
			void* input = kmalloc(sizeof(struct pcn_kmsg_message), GFP_KERNEL);
	...
	int pci_kmsg_send_long(unsigned int dest_cpu, struct pcn_kmsg_message *lmsg, unsigned int payload_size)
	{
		...
		pcn_msg = lmsg;
		ivshmem_kmsg_send(1, pcn_msg, sizeof(struct pcn_kmsg_message));
	...
	void test_func(void)
	{
		...
		for (i = 0; i < NUM_MSGS; i++) {
			pci_kmsg_send_long(0, (struct pcn_kmsg_message*)msg,
					   payload_size);
